...
writeShellApplication {
  ...
  text = ''
        ...
        FIRST_TURN=true
        TTS_SERVER_PID=""
        trap 'rm -f "$TMPFILE"; [[ -n "$TTS_SERVER_PID" ]] && kill "$TTS_SERVER_PID" 2>/dev/null; printf "\nGoodbye.\n" >&2' EXIT

        VOICE_PROMPT=${lib.escapeShellArg voicePrompt}
        MLX_SPEAK_PORT="''${MLX_SPEAK_PORT:-47732}"
    ...
    PYEOF
        ...
        # -- start TTS server if models are present and server is not running -----
        if [[ -d "$MLX_SPEECH_MODELS/openmoss/moss_tts_local/mlx-int8" ]]; then
          if tts_server_running; then
            step "TTS server already running on port $MLX_SPEAK_PORT"
          else
            step "Starting TTS server (loading model, ~10s)..."
            MLX_SPEECH_MODELS="$MLX_SPEECH_MODELS" \
              MLX_SPEAK_PORT="$MLX_SPEAK_PORT" \
              mlx-speak-server >>/tmp/mlx-speak-server.log 2>&1 &
            TTS_SERVER_PID=$!
            SERVER_READY=false
            ...
            if $SERVER_READY; then
              step "TTS server ready (PID $TTS_SERVER_PID)"
            else
              step "TTS server did not start in 30s — will fall back to say (check /tmp/mlx-speak-server.log)"
              TTS_SERVER_PID=""
            fi
          fi
        else
          step 'TTS models not found — using say (run "init" to download)'
        fi
        # -------------------------------------------------------------------------
        ...
        while true; do
          ...
          printf 'Claude: %s\n\n' "$RESPONSE" >&2
          if [[ -d "$MLX_SPEECH_MODELS/openmoss/moss_tts_local/mlx-int8" ]]; then
            MLX_SPEECH_MODELS="$MLX_SPEECH_MODELS" MLX_SPEAK_PORT="$MLX_SPEAK_PORT" \
              mlx-speak "$RESPONSE" || {
  ...
  meta = with lib; {
    description = "Voice assistant: mic → parakeet-mlx → Claude → mlx-speak-server";
