...
let
  ...
  importEmacsConfig =
    ...
    in
    if builtins.isFunction imported then imported { modalEditing = cfg.modalEditing; } else imported;
  emacsUsePackageConfigs = lib.foldl' (
    acc: path: acc // importEmacsConfig path
  ) { } emacsConfigNixFiles;
in
with lib;
{
          ...
          programs.emacs = {
            ...
            init = {
              ...
              postlude = ''
                ...
                (defun pragmatapro/prettify-symbols-compose-p (start end _match)
                  "PragmataPro modification of `prettify-symbols-default-compose-p'.
                Only difference is to compose symbols in comments as well. See
                `prettify-symbols-default-compose-p' for documentation on START and END."
                ...
                (defun pragmatapro/setup-prettify-hook ()
                  "Setup `prettify-symbols-mode' settings for use with PragmataPro."
                  (dolist (sym pragmatapro/prettify-symbols-alist)
              ...
              usePackage = emacsUsePackageConfigs // {
                # pratice emacs-fu
                disable-mouse = {
                  enable = false;
                  config = ''
