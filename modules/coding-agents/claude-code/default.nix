...
in
{
  ...
  # On NixOS, `lib.mkIf false { launchd = ... }` still surfaces `launchd` to the checker
  # and fails ("option `launchd' does not exist"). Only attach a launchd fragment on Darwin;
  # `lib.optional false _` does not evaluate the element, so Linux never sees `launchd`.
  config = lib.mkMerge (
    [
      (lib.mkIf cfg.enable {
        home-manager.users.${config.my.username} = {
          ...
          mcp-servers.programs.sequential-thinking = {
            enable = true;
            # mcp-servers-nix HM module resolves defaults from its own `servers`
            # overlay, not nixpkgs; use our fixed package from common.nix.
    ...
    ++ lib.optional isDarwin {
      ...
      launchd.user.agents.claude-fork-bomb-watchdog = lib.mkIf cfg.enable {
        serviceConfig = {
