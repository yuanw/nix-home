...
in
{
  home.activation.librewolfResetRsyncedProfile = lib.mkIf (program == "librewolf") (
    hm.config.lib.dag.entryBefore [ "checkLinkTargets" ] ''
      profile="$HOME/${profilesPath}/home"
      if [ -L "$profile/.keep" ] && readlink "$profile/.keep" 2>/dev/null | grep -q 'Firefox/Profiles'; then
        echo "gecko-home: removing rsynced Firefox profile from LibreWolf at $profile" >&2
        rm -rf "$profile"
      fi
    ''
  );
  ...
  // lib.optionalAttrs (!isDarwin) {
    ".mozilla/firefox/profiles.ini".text = ''
      ...
      IsRelative=0
      Name=home
      Path=${config.my.homeDirectory}/${
        if program == "firefox" then ".config/mozilla/firefox/home" else ".librewolf/home"
      }
    '';
  };
  ...
  programs.${program} = {
    enable = true;
    package = cfg.pkg;
    betterfox = lib.mkIf (program == "firefox") {
      enable = true;
    };
  };
  ...
  programs.${program}.policies = lib.mkMerge [
    {
  ...
  # programs.${program}.nativeMessagingHosts = [
  ...
  # Pattern: https://github.com/js0ny/nixcfgs/blob/cac17a65f58a2a90a9c0462579042bf79dfa8c46/helper/mergetools.nix
  mergetools = lib.mkIf (keybindingsCfg.enable && keybindingsCfg.settings != { }) {
    "${program}-extension-keybindings" = {
      target = "${config.my.homeDirectory}/${profilesPath}/home/extension-settings.json";
      format = "json";
      force = true;
      backup = true;
      settings = {
