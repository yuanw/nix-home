Save the current Claude session as a journal entry using emacsclient and denote-journal.
...
2. Compose an elisp expression to append the session to today's denote journal:
   ...
   - Insert a top-level org heading with timestamp: `* Claude Session - HH:MM\n`
...
3. Run the elisp via emacsclient:
   ...
   emacsclient --eval "(progn
     ...
     (with-current-buffer (current-buffer)
       (goto-char (point-max))
       (insert \"\n* Claude Session - \" (format-time-string \"%H:%M\") \"\n\")
