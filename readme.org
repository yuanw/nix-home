...
** Prerequisites for GitHub CLI Git Credential Helper
The Linux hosts use GitHub CLI (~gh~) as the git credential helper. To set it up:

1. Authenticate with GitHub CLI:
   ~gh auth login~
2. Follow the prompts to authenticate via browser or token
3. Git operations will now use your GitHub CLI credentials automatically
